...
7. **Score** — the blind council: one frontier model per major lab independently scores every key factor (−5…+5 alignment with the thesis, plus how decisive the factor is) and the thesis overall, and may suggest missing factors. A script assembles the sheets into the report's Conclusions — every member's scores are shown side by side (no medians, no averaging) along with each member's 0–100 "factor coverage" judgment of whether the factors suffice to call the thesis; disagreement is shown deliberately. The council reads the report body only — the Conclusions are stripped from its packet, so no member ever sees the previous verdict or another member's answers.

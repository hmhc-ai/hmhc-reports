...
5. **Site/design session scope.** The site session designs and builds the **site module only**: `.github/site/**` (generator `build_site.py` · spec `build-site.md` · design system `style.css`), `.github/workflows/pages.yml` (the deploy workflow), and `pipeline/sg-banks/index.md` **changelog entries only**, describing its site changes. Everything else — `reports/`, the rest of `pipeline/**`, `AGENTS.md`, `PERPLEXITY.md`, other workflows, `reports.json`, tag/version files — is out of its write scope. Content flows **one way**: the pipeline produces content, the site renders it; a rendering problem is fixed in the renderer, a content problem is routed back to the author — never patched in source markdown/CSV. Site work rides `site/*` branches, and CI enforces the fence: `.github/scripts/site_scope_guard.py` (docs-lint workflow) fails any `site/*` PR touching a file outside the list above.

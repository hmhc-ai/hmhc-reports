...
> **Artifact:** `pipeline/sg-banks/method/2-script-build-gaps.md` — the specification for `build_gaps.py`; keep the two in sync.
...
| **Executor** | **No model — a deterministic script:** `python3 pipeline/sg-banks/method/2-script-build-gaps.py`; `--check` verifies reproducibility (CI runs it on every PR). |

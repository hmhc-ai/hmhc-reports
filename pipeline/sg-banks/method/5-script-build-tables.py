...
Usage:  python3 pipeline/sg-banks/method/5-script-build-tables.py [--check]
        --check: build to a temp string, compare numeric cells against the
                 committed data/tables.md, exit non-zero on any difference.

Spec: pipeline/sg-banks/method/5-script-build-tables.md (formats, gates, footnotes).
"""
import csv, sys, re
from decimal import Decimal, ROUND_HALF_EVEN
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]          # pipeline/sg-banks (this file lives in method/)
...
def build():
    ...
    head = ["# SG Banks — Tables (generated table-block artifact)", "",
            "> **Artifact:** `pipeline/sg-banks/data/tables.md` — sole output of `pipeline/sg-banks/method/5-script-build-tables.py` "
            "(the deterministic Build-Tables script; spec in `method/5-script-build-tables.md`). Consumed by the Build-Report step "
            "(`pipeline/sg-banks/method/6-ai-build-report.md`).",
            f"> **Provenance:** every number is a deterministic transform of reconciled `pipeline/sg-banks/data/ledger.csv` "
            f"({len(ROWS)} data rows: {status_line}). Rerun `python3 pipeline/sg-banks/method/5-script-build-tables.py` to regenerate; "
